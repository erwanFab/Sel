...
echo '<html><body>
<form method="post" action="http://SERVEUR WEB/wordpress/?page_id=1135 ">';
...
<label>Activation: <input type="checkbox" name="actif"/></label>  </br>
...
$query = "UPDATE `sel_adherent` SET `actif` = '1' WHERE `id_adherent` = '".$adherent."'";
...
$query="SELECT id_adherent,identifiant,ville,actif from sel_adherent WHERE actif ='1'";
						...
						if (!$query) {
									...
									echo '<html><body><form method="post" action="http://SERVER WEB/wordpress/?page_id=1133 ">';
						...
						if($total) {
										...
										while($row = mysql_fetch_array($result)) 
										{
												...
												echo '<td bgcolor="#CCCCCC"><a target="_blank" href="http://SERVER BEV/wordpress/?page_id=393  & identifiant='.htmlspecialchars ($row['identifiant']).'  ">'.htmlspecialchars ($row["identifiant"]).'</a></td>';
